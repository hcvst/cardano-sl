...
getHeadersRange depthLimitM older newer = runExceptT $ do
    ...
    -- This is [oldest..newest) headers, newest first
    ...
    -- It's safe to use 'unsafeHead' here after the last check.
    let firstElem = allExceptNewest ^. _Wrapped . to unsafeHead
    when (newerHd ^. prevBlockL . headerHashG /= firstElem) $
        throwError $
    ...
    let conv =
           ...
           nonEmpty .
           (newer:)
    pure $ toOldestFirst $ allExceptNewest & _Wrapped %~ conv
  where
